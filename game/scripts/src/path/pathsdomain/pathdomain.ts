...
export class PathDomain extends Path {
    ...
    constructor(entity: CBaseEntity) {
        ...
        GameRules.EventManager.Register("Event_PlayerRoundBefore", (event: { typeGameState: number }) => this.onEvent_PlayerRoundBefore(event), this, -987654321)
    ...
    atkCity(oPlayer: Player) {
        if (!this.m_tabENPC[0] || this.m_tabENPC[0].IsNull()
            || this.m_nPlayerIDGCLD || this.m_tabENPC[0].m_bBattle) {
            return
        }
        this.m_tEventIDGCLD = []
        ...
        // 监听行为终止
        this.m_tEventIDGCLD.push(GameRules.EventManager.Register("Event_ActionStop", (event: {
    ...
    atkCityEnd(bWin: boolean, bMoveBack?: boolean) {
        ...
        if (tGCLDEnd.bWin) {
            ...
            AHMC.CreateParticle("particles/units/heroes/hero_legion_commander/legion_commander_duel_victory.vpcf"
                , ParticleAttachment.ABSORIGIN, false, oPlayer.m_eHero)
