...
export class PathManager {
    ...
    getPathByID(nID: number) {
        for (let index = 0; index < this.m_tabPaths.length; index++) {
            if (nID == this.m_tabPaths[index].m_nID) return this.m_tabPaths[index];
        }
    }

    /**获取路径对象 by pathClass */
    getPathByClass(targetClass: any) {
        let tabPath: Path[] = [];
        for (let index = 0; index < this.m_tabPaths.length; index++) {
            if (this.m_tabPaths[index] instanceof targetClass) {
                tabPath.push(this.m_tabPaths[index]);
            }
        }
        return tabPath;
    ...
    /**获取可占领领地 */
    getNoOwnerPaths() {
        const paths = GameRules.PathManager.getPathByClass(PathDomain).concat(GameRules.PathManager.getPathByClass(PathTP));
        return paths.filter(path => {
            if (!path['m_nOwnerID']) return true;
        });
    }
    ...
    getOwnerPaths() {
        const paths = GameRules.PathManager.getPathByClass(PathDomain).concat(GameRules.PathManager.getPathByClass(PathTP));
        return paths.filter(path => {
            if (path['m_nOwnerID'] && path['m_nOwnerID'] > -1) return true;
        });
    }

    /**获取平均领地数量 */
    getPathCountAvg() {
        return math.floor(this.getOwnerPaths().length / GameRules.PlayerManager.getAlivePlayerCount());
    }
    ...
    moveStop(entity: CDOTA_BaseNPC_Hero, bSuccess: boolean) {
        ...
        if (tMoveData.funCallBack != null) tMoveData.funCallBack(bSuccess);
    }
}
