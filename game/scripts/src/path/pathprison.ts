...
export class PathPrison extends Path {
    ...
    constructor(entity: CBaseEntity) {
        ...
        GameRules.EventManager.Register("Event_Roll", (event: {
            ...
            nNum2: number
            player: Player
        }) => this.onEvent_Roll(event), this)
        GameRules.EventManager.Register("Event_PlayerRoundBegin", (event: {
            oPlayer: Player,
            bRoll: boolean,
            bIgnore?: boolean
        }) => this.onEvent_PlayerRoundBegin(event), this)
    }
    ...
    setInPrison(player: Player) {
        ...
        // 监听攻城失败给兵卒更新监狱buff
        tEventID.push(GameRules.EventManager.Register("Event_PrisonOut", (event: { player: Player }) => {
    ...
    isInPrison(nEIndex: EntityIndex) {
        for (const hero of this.m_tabENPC) {
            if (hero.GetEntityIndex() == nEIndex)
                return true
        }
        return false
    }
