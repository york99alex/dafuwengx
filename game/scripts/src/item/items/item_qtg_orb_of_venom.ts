        ...
        if (IsServer()) {
            if (event.attacker == this.GetParent()) {
                ...
                if (
                    ...
                    // 远程 range
                    AHMC.AddNewModifier(event.target, event.attacker, this.GetAbility(), modifier_item_qtg_orb_of_venom_range.name, {
                        duration: this.poison_duration,
                    });
                } else {
                    // 近战 melee
                    AHMC.AddNewModifier(event.target, event.attacker, this.GetAbility(), modifier_item_qtg_orb_of_venom_melee.name, {
                        duration: this.poison_duration,
                    });
                }
            }
