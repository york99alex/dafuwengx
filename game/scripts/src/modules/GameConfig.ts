...
export class GameConfig {
    ...
    registerEvent() {
        ...
        // 监听攻击导致的金钱变化
        GameRules.EventManager.Register('Event_ChangeGold_Atk', (event: { nGold: number; player: Player }) => this.onEvent_ChangeGold(event), this);

        // 监听玩家死亡
        GameRules.EventManager.Register('Event_PlayerDie', (event: { player: Player }) => this.onEvent_PlayerDie(event), this, -1000);
    }
    ...
    /**处理攻城略地 */
    processGCLD(tabData: { nPlayerID: number; typeOprt: number; nPathID?: number; nRequest?: number }) {
        // 删除可操作
        const tabOprt = this.checkOprt(tabData, true);
    ...
    onEvent_PlayerDie(event: { player: Player }) {
        ...
        // 剩余操作出来
        if (this.m_nOrderID == event.player.m_nPlayerID && this.m_typeState != GS_DeathClearing) {
            // 移除操作
            for (let i = 0; i < this.m_tabOprtCan.length; i++) {
                this.m_tabOprtCan = this.m_tabOprtCan.filter(v => v.nPlayerID !== event.player.m_nPlayerID);
            }
            if (nAlive > 1) {
        ...
        // 改变首位玩家
        print('this.m_nOrderFirst == event.player.m_nPlayerID:', this.m_nOrderFirst == event.player.m_nPlayerID);
    ...
    /**玩家金币变化 */
    onEvent_ChangeGold(event: { nGold: number; player: Player }) {
    ...
    onEvent_Roll(event: { bIgnore: 0 | 1; nNum1: number; nNum2: number; player: Player }) {
        ...
        oPlayer.moveToPath(pathDes, (bSuccess: boolean) => {
            ...
            // 触发移动结束事件
            if (this.m_typeState === GS_Move || this.m_typeState === GS_DeathClearing) {
                // GSMOVE_Exit()结束移动
                GameRules.GameLoop.Timer(() => {
                    GameRules.GameLoop.GameStateService.send('towaitoprt');
                    return null;
            ...
            if (event.nNum1 == event.nNum2 && bit.band(oPlayer.m_nPlayerState, PS_InPrison + PS_AtkMonster) == 0) {
                // 豹子,发送roll点操作
                this.broadcastOprt({
                    typeOprt: TypeOprt.TO_Roll,
